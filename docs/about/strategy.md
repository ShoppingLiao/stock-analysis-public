...
這是一個**台股短線「強勢股回檔買點」進場策略**,經過 **9.5 年全市場、6033 筆 trigger 完整回測驗證**。
...
| 指標 | 數值 | 說明 |
|------|------|------|
| 總 trigger 數 | **6033** | 9.5 年全市場掃描 |
| 已結案 | 5958 | 觀察期內未出場僅 75 筆 (1.2%) |
| 勝率 | **33.1%** | wins=1995, losses=4038 |
| 平均獲利 | **+9.73%** | 含尾部飆股(最大 +162%) |
| 平均虧損 | **-2.85%** | Stage A 緊停損保命 |
| **每筆期望值** | **+1.31%** | 扣 0.4% 手續費後仍有 +0.9% |
| 中位數報酬 | -1.57% | 多數單筆小虧,獲利集中在右尾 |
| 平均持有 | 6.3 天 | 短線策略 |
| 平均 MFE | +27.09% | 進場後潛在最高漲幅 |
| 平均 MAE | -13.11% | 「不停損」會看到的最大跌幅 — **證明停損很關鍵** |
...
> **55.9% 觸及 Stage A 停損** — 大多數 trigger 沒站穩 +5%,這是「短線回檔」的合理結果。
> **12.6% 走到 Stage D**(漲超過 +15% 後從高點回檔 12% 出場)— 是貢獻 +9.73% 平均獲利的核心來源。

### 策略穩定性

![策略穩定性 — 滾動勝率與期望值](../assets/backtest/rolling_stability.png)

> 上圖每個點是一筆 trigger(綠贏 / 紅輸)。下圖是滾動 200 筆視窗的勝率(綠線)與期望值(藍線)。
...
1. **回測 ≠ 實際操作**
   真實滑價、心理因素、執行紀律都會降低勝率。期望值 +1.31% 扣手續費(~0.4%/筆)後約 **+0.9%/筆**。

2. **熊市要空手或降部位**
   2022 熊市單年期望值轉負(-1.2%)— 多頭排列只是必要條件,大盤環境決定整體勝率。
