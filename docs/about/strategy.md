...
這是一個**台股短線「強勢股回檔買點」進場策略**,經過 **9.5 年全市場、7357 筆 trigger 完整回測驗證**。
...
| 指標 | 數值 | 說明 |
|------|------|------|
| 總 trigger 數 | **7357** | 9.5 年全市場掃描(2052 檔) |
| 已結案 | 7279 | 觀察期內未出場僅 78 筆 (1.1%) |
| 勝率 | **32.6%** | wins=2399, losses=4958 |
| 平均獲利 | **+9.49%** | 含尾部飆股(最大 +162%) |
| 平均虧損 | **-2.81%** | Stage A 緊停損保命 |
| **每筆期望值** | **+1.20%** | 扣 0.4% 手續費後仍有 +0.80% |
| 中位數報酬 | -1.57% | 多數單筆小虧,獲利集中在右尾 |
| 平均持有 | 6.4 天 | 短線策略 |
| 平均 MFE | +26.53% | 進場後潛在最高漲幅 |
| 平均 MAE | -13.00% | 「不停損」會看到的最大跌幅 — **證明停損很關鍵** |
...
> **57.1% 觸及 Stage A 停損** — 大多數 trigger 沒站穩 +5%,這是「短線回檔」的合理結果。
> **12.1% 走到 Stage D**(漲超過 +15% 後從高點回檔 12% 出場)— 是貢獻 +9.49% 平均獲利的核心來源。
...
1. **回測 ≠ 實際操作**
   真實滑價、心理因素、執行紀律都會降低勝率。期望值 +1.20% 扣手續費(~0.4%/筆)後約 **+0.80%/筆**。
